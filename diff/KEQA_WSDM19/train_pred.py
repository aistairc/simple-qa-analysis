...
for line in open(os.path.join(args.map2id_dir, 'relation2id.txt'), 'r'):
    items = line.strip().split("\t")
    pre_dic[items[0]] = int(items[1])
# Embedding for predicates
predicates_emb = torch.from_numpy(np.fromfile(args.pred_emb, dtype=np.float32).reshape((len(pre_dic), args.embed_dim)))
